...
class MyGNN(tf.keras.Model):
    ...
    def call(self, input_):
        ...
        w = tf.reshape(tf.math.exp(-10*d), (x_shape[0], x_shape[1], 1))

        x = tf.reduce_sum(x * w * x_mask, axis=1)
        
        # tf.print(tf.shape(x), output_stream=sys.stderr)
...
def main(cfg: DictConfig) -> None:
    ...
    with mlflow.start_run(**run_kwargs) as active_run:
        ...
        mlflow.log_artifact(to_absolute_path("Training_DisTauTag_SNNv0.py"), 'input_cfg')
        mlflow.log_artifact(to_absolute_path("../commonReco.py"), 'input_cfg')
        mlflow.log_artifacts('.hydra', 'input_cfg/hydra')
        mlflow.log_artifact('Training_DisTauTag_SNNv0.log', 'input_cfg/hydra')
