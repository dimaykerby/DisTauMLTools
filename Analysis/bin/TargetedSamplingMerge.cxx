...
struct Arguments {
    ...
    run::Argument<std::string> tau_ratio{"tau-ratio", "ratio of tau types in the final spectrum "
                                                      "(if to take all taus of type tau => tau:-1 )"};
...
struct EntryDesc {
              ...
              const bool overflow_job)
    {
        ...
        bool is_matching_spectrum = false;
        ...
        path p (base_spectrum_dir);
        directory_iterator end_itr;
        for (directory_iterator itr(p); itr != end_itr; ++itr)
        {
          std::string current_file = itr->path().string();
          if(!regex_match(current_file , dir_pattern )) continue;
          if(!regex_match(current_file, root_pattern)) continue;
          is_matching_spectrum = true;
          spectrum_files.insert(current_file);
        }
        ...
        if(!is_matching_spectrum){
          throw exception("No spectrum are found for entry '%1%' with pattern '%2%'")
              % name % file_pattern_str;
        }
...
public:
                  ...
                  ttypes(tau_types_)
    {
      ...
      for(JetType type: ttypes)
      {
        ...
        ttype_prob[type] = 
          std::make_shared<TH2D>(name,name,eta_bins.size()-1, &eta_bins[0], pt_bins.size()-1, &pt_bins[0]);
        ttype_entries[type] = 
          std::make_shared<TH2D>(name,name,eta_bins.size()-1,&eta_bins[0], pt_bins.size()-1,&pt_bins[0]);
      }
      n_entries = 0;
        ...
        for(int i_x = 1; i_x<=ratio_h->GetNbinsX(); i_x++)
        {
          for(int i_y = 1; i_y<=ratio_h->GetNbinsY(); i_y++)
          {
            ...
            ratio_h->SetBinContent(i_x,i_y,
              ttype_entries.at(signal_type)->GetBinContent(i_x,i_y)/ttype_entries.at(type)->GetBinContent(i_x,i_y));
	        }
        }

        Int_t x_max,y_max,z_max;
        ratio_h->GetBinXYZ(ratio_h->GetMaximumBin(), x_max, y_max, z_max);

        if(ratio_h->GetBinContent(x_max, y_max)==0)
          throw exception("Histogram '%1%' in '%4%' is empty.")
          % ToString(type) % groupname;

        ratio_h->Scale(1.0/ratio_h->GetBinContent(x_max,y_max));
...
private:
    ...
    std::map<JetType, Double_t> ttype_prob; // pair<type, probability> (summed up per all dataset)
...
class TargetedSamplingMerge {
public:
    using Tau = tau_tuple::Tau;
    using TauTuple = tau_tuple::TauTuple;
    using Generator = DataSetProcessor::Generator;

    TargetedSamplingMerge(const Arguments& _args) :
        ...
        max_entries(args.max_entries()), parity(args.parity()), compression(compAlg(args.compression_algo()))
    {
      ...
      if(verbose) {
        for (auto dsc: all_entries) {
          ...
          std::cout << "spectrums: " << dsc.spectrum_files.size() << std::endl;
          for (auto sp: dsc.spectrum_files)
    ...
    void Run()
    {
      ...
      const std::vector<EntryDesc>& entry_list = global_entry;
      const std::string& file_name = args.output();
...
PROGRAM_MAIN(analysis::TargetedSamplingMerge, analysis::Arguments)
